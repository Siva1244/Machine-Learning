...
for i=1:1:m
  ...
  del3=a3-y_matrix(i,:)';
  del2=(Theta2(:,2:end)'*del3).*(sigmoidGradient([Theta1*X(i,:)']));
 
  J= J + (-y_matrix(i,:)*log(a3) - (1-y_matrix(i,:))*log(1-a3));
...
Theta1_grad=(1/m).*Theta1_grad + (lambda/m).*Th1;
Theta2_grad=(1/m).*Theta2_grad + (lambda/m).*Th2;
